...
#
#     $ RN_ADJECTIVES=yes RN_SEPERATOR="_" curl -L https://bit.ly/294762 | bash
#     boring_wozniak  
#   
...
RN_ADJECTIVES=${RN_ADJECTIVES:="yes"}

# default to "-"
RN_SEPERATOR=${RN_SEPERATOR:="-"}
...
# typically i don't like shelling out, but $RANDOM? eh...
#

if [[ "${RN_ADJECTIVES}" != "no" ]]; then
    __leftOut=( $(shuf -e "${__left[@]}") )
    __leftOut+="${RN_SEPERATOR}"
fi

__rightOut=( $(shuf -e "${__right[@]}") )

# output
echo -n "${__leftOut}${__rightOut}"

# cleanup
unset __left __right __leftOut __rightOut
